        ...
        <div className="w-9/12 ml-4">
          {categories.map((category, index1) => 
            <div key={index1} className="mb-6">
              <h3 className="uppercase font-medium text-xl mb-3">{category.name}</h3>
              <div className="grid grid-cols-3 gap-8">
                {category.products
                  .filter((product) => product.isPublic === true)
                  .map((product, index2) => (
                    <Link key={index2} to={"/menu/" + product._id}>
                    <div key={index2} className="rounded-lg bg-white overflow-hidden shadow-md">
                        <div>
                        ...
                        <div className="py-2 px-6 uppercase font-medium">
                            ...
                            <p className="my-2 text-red-700">{product.discount > 0 ? product.discount : product.price} vnd</p>
                            <Link to="/dat-mon"><button className="py-1 px-3 bg-red-700 text-white uppercase font-medium rounded-md">dat mon</button></Link>
                        </div>
                    </div>
                    </Link>
                  ))}
