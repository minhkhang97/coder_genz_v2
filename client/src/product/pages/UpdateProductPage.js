...
const UpdateProductPage = () => {
    ...
    if (status === "loading" || categoriesReducer.status === 'loading') return <div>loading</div>;
    if (status === "failed" || categoriesReducer.status === 'failed') return <div>failed</div>;
    return (
      <div>
        <div class="mb-1 mt-5">
        ...
        <ProductDetail product={product} categories={categoriesReducer.categories}/>
      </div>
    );
}
