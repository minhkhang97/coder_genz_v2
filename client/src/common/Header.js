...
const Header = () => {
  ...
  return (
    <div>
      ...
      {isLogin && (
        <div className="fixed bg-white inset-x-1/4 w-1/2 px-6 py-4 rounded-lg shadow-md">
          <p className="uppercase font-medium text-xl text-center">dang nhap</p>
          <form
            onSubmit={async (e) => {
              e.preventDefault();
              const res = await apiHandler.post('/customer/login', {email, password});
              console.log(res);
                if (!res.success) {
                  setNoti([res.error]);
                }
                    ...
                    const data = await apiHandler.get('/customer/read');
                    console.log(data);
                }
            }}
          >
